...
fn caos(args: &[&str]) -> Result<String> {
    ...
    if !output.status.success() {
        return Err(format!("caos {}: {}", args[0], String::from_utf8_lossy(&output.stderr)).into());
    }
    Ok(String::from_utf8(output.stdout)?.trim().to_owned())
}
fn get(path: &str) -> Result<()> { caos(&["get", path])?; Ok(()) }
fn hash(path: &str) -> Result<String> { caos(&["hash", path]) }
fn arg(name: &str) -> String { format!("/cas/args/{name}") }
...
fn required(name: &str) -> Result<String> { optional(name)?.ok_or_else(|| format!("missing {name}").into()) }
fn read_json<T: DeserializeOwned>(path: &str) -> Result<T> {
    get(path)?;
    Ok(serde_json::from_slice(&fs::read(path)?)?)
}
fn curry(stage: &str, args: &[String]) -> Result<String> {
    let mut tokens = vec!["curry".into(), "--base:@=/cas/args/base".into(), format!("--stage={stage}")];
...
fn run_then(input: &str, run: &str, then: &str) -> Result<()> {
    caos(&["run-then", input, &format!("--run:hash={run}"), &format!("--then:hash={then}")])?;
    Ok(())
}
...
fn failure(message: &str) -> Result<()> {
    ...
    fs::write(temp.path().join("results.json"), serde_json::to_vec(&json!({"error":message}))?)?;
...
fn start() -> Result<()> {
    ...
    let limit = match optional("limit")?.unwrap_or_else(|| "10".into()).parse::<usize>() {
        Ok(limit) if (1..=100).contains(&limit) => limit,
        _ => return failure("limit must be between 1 and 100"),
    };
    let chunk_chars = match optional("chunk-chars")?.unwrap_or_else(|| DEFAULT_CHUNK_CHARS.to_string()).parse::<usize>() {
    ...
    if scope != "." && (scope.is_empty() || Path::new(&scope).components()
        .any(|part| !matches!(part, std::path::Component::Normal(_)))) {
        return failure("path must be a relative source path without parent traversal");
    }
    let source_tree = hash(&arg("in"))?;
    if caos(&["resolve", &source_tree, &scope, "/cas/selected"]).is_err() {
        return failure("selected path is absent from the source snapshot");
    }
    let index = curry("index", &[format!("--chunk-chars={chunk_chars}")])?;
    let query = curry("query", &[format!("--query={query}"), format!("--limit={limit}"),
        format!("--source-tree={source_tree}"), format!("--scope={scope}")])?;
    run_then("/cas/selected", &index, &query)
}
fn collect(
    source: &str, relative: &str, files: &mut Vec<FileLocation>,
    blobs: &mut BTreeMap<String, String>, skipped: &mut Vec<String>,
) -> Result<()> {
    get(source)?;
    ...
    if path.is_dir() {
        ...
        for entry in entries {
            let name = entry.file_name().into_string().map_err(|_| "non-UTF8 source path")?;
            ...
            if child.is_dir() || selected_file(&next) {
                collect(child.to_str().ok_or("non-UTF8 CAS path")?, &next, files, blobs, skipped)?;
            }
        }
    } else {
        let bytes = fs::read(path)?;
        if bytes.len() > MAX_FILE_BYTES || bytes.contains(&0) || std::str::from_utf8(&bytes).is_err() {
            skipped.push(format!("{relative}: binary, invalid UTF-8, or over 256 KiB"));
            return Ok(());
        }
        let blob = hash(source)?;
        files.push(FileLocation { path: relative.to_owned(), blob: blob.clone() });
...
fn index() -> Result<()> {
    ...
    let mut manifest = Manifest {
        source_tree: hash(&arg("in"))?, files: Vec::new(), skipped: Vec::new(),
        model: embedding_identity()?, dimension: 384, chunk_chars,
    };
    let mut blobs = BTreeMap::new();
    collect(&arg("in"), "", &mut manifest.files, &mut blobs, &mut manifest.skipped)?;
    let temp = tempfile::tempdir()?;
    let inputs = temp.path().join("inputs");
    fs::create_dir(&inputs)?;
    for (blob, path) in blobs { symlink(path, inputs.join(blob))?; }
    ...
    caos(&["map-then", "/cas/files", &format!("--map:hash={embed}"),
        &format!("--then:hash={assemble}"), "--max-parallel=2"])?;
    Ok(())
}
fn embed_file() -> Result<()> {
    get(&arg("in"))?;
    let chunks = embedded_chunks(&fs::read_to_string(arg("in"))?, required("chunk-chars")?.parse()?)?;
    let temp = tempfile::tempdir()?;
...
fn search() -> Result<()> {
    // This eager bridge is retained as the benchmark baseline before lazy reads.
    caos(&["get", "-r", &arg("index")])?;
    let vector: Vec<f32> = read_json(&arg("result"))?;
    let mut result = query_directory(Path::new(&arg("index")), vector,
        required("limit")?.parse()?, &embedding_identity()?)?;
    result["source_tree"] = Value::String(required("source-tree")?);
    let scope = required("scope")?;
    let mut report = String::new();
    for (rank, hit) in result["hits"].as_array_mut().ok_or("query hits missing")?.iter_mut().enumerate() {
        let relative = hit["payload"]["path"].as_str().ok_or("hit path missing")?;
        let path = if scope == "." { relative.to_owned() } else if relative.is_empty() {
            scope.clone()
        } else { format!("{scope}/{relative}") };
        hit["payload"]["path"] = Value::String(path.clone());
        report.push_str(&format!("{}. {}:{}-{} (score {})\n{}\n\n", rank + 1, path,
            hit["payload"]["start_line"], hit["payload"]["end_line"], hit["score"],
            hit["payload"]["document"].as_str().unwrap_or("")));
    }
    report.push_str(&format!("{} matches in source {} (index {}).\n",
        result["hits"].as_array().ok_or("query hits missing")?.len(),
        result["source_tree"].as_str().unwrap_or(""), result["index_root"].as_str().unwrap_or("")));
    let temp = tempfile::tempdir()?;
    fs::write(temp.path().join("results.json"), serde_json::to_vec(&result)?)?;
    fs::write(temp.path().join("report"), report)?;
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => { eprintln!("git-vdb-caos: {error}"); ExitCode::FAILURE }
    }
}
